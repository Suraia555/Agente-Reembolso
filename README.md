...
```
## 🔒 Enterprise JWT & Webhook Testing
To execute requests on protected operational endpoints via Swagger UI (`/docs`), inject the bearer authorization token generated by the `/auth/login` gateway into the secure HTTP header layer:
```http
Authorization: Bearer <your_supabase_jwt_token>
```

The system includes an active **Anti-Fraud Billing Shield**. If a user accumulates $\ge 3$ unpaid carrier refund claim receipts, the processing engine halts dynamically to preserve OpenAI API token consumption and safeguard financial margins.
